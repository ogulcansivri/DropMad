...
```mermaid
graph LR
    A["1. Drag File(s)"] --> B["2. Drop on DropMad"]
...
**Q: Does removing an item from DropMad delete my original file?**  
> **No.** DropMad only holds temporary references. Removing an item from the shelf leaves your original file completely safe.

**Q: Does DropMad use a lot of battery or CPU?**  
> **Zero.** DropMad is written in 100% native Swift & SwiftUI with AppKit. It uses virtually 0% CPU when idle and has zero background polling.

**Q: Why doesn't DropMad appear in my Dock?**  
> DropMad runs as a clean Menu Bar accessory (`LSUIElement`) to keep your macOS Dock free of clutter.
...
```mermaid
graph LR
    A["1. Dosyaları Tut"] --> B["2. DropMad'e Bırak"]
...
> **Hayır.** DropMad sadece geçici referans tutar. Raftan kaldırmak veya rafı temizlemek asıl dosyalarınıza kesinlikle zarar vermez.

**S: Pil veya işlemci harcar mı?**  
> **Sıfıra yakın.** DropMad saf Swift & SwiftUI ile yazılmıştır. Boşta beklerken %0 işlemci kullanır ve pilinizi tüketmez.

**S: Neden Dock'ta görünmüyor?**  
> DropMad, Dock kalabalığı yapmamak için bir menü çubuğu aracı (`LSUIElement`) olarak hafif bir şekilde çalışır.
